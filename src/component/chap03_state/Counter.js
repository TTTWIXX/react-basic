...
const Counter = () => {
    ...
    const[nick,setNick]=useState('익명'); //(0) 초기값
    ...
    /*
        ...
        - 상태값이 변경될 때마다 다시 호출
        - 만약에 첫 렌더링 시에만 uesEffect를 호출하고 싶다면
          두번째 파라미터로 빈 배열(의존성 배열)을 넣으면 된다.
        - 의존성 배열에 상태변수를 넣으면 해당 상태변수가 업데이트 될 때 마다
          uesEffect가 재 호출됨.
    */
   
        useEffect(()=>{
            // 화면이 처음 렌더링 될 때 서버 api호출하는 경우
            // 처음 1번만 호출후에 또 할 필요가 있을까??
       console.log('3. uesEffect Call!!');
